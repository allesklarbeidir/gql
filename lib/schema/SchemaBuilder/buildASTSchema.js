      ...
      case 'TypeExtensionDefinition':
        {
          var _ret = function () {
            var _d = d.definition;
            var name = _d.name.value;
            if (!nodeMap[name]) {
              typeDefs.push(_d);
              nodeMap[name] = _d;
            } else {
              var existingTd = typeDefs.filter(function (td) {
                return td.name.value === name;
              })[0] || null;
              if (existingTd) {
                var _ref;

                existingTd.fields = (_ref = []).concat.apply(_ref, [].concat((0, _toConsumableArray3.default)(existingTd.fields), [_d.fields || []]));
              }
            }
